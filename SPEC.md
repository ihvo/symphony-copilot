...
5. `Integration Layer` (GitHub Issues adapter)
...
- Issue tracker API (GitHub Issues for `tracker.kind: github` in this specification version).
- Local filesystem for workspaces and logs.
- OPTIONAL workspace population tooling (for example Git CLI, if used).
- Coding-agent executable that supports the targeted Copilot SDK mode.
...
- `thread_id` (string)
- `turn_id` (string)
- `copilot_pid` (string or null)
- `last_copilot_event` (string/enum or null)
- `last_copilot_timestamp` (timestamp or null)
- `last_copilot_message` (summarized payload)
...
For Copilot SDK-owned config values such as `approval_policy`, `thread_sandbox`, and
`turn_sandbox_policy`, supported values are defined by the targeted Copilot SDK version.
Implementors SHOULD treat them as pass-through Copilot SDK config values rather than relying on a
hand-maintained enum in this spec. To inspect the installed Copilot SDK schema, run
`copilot-sdk generate-json-schema --out <dir>` and inspect the relevant definitions referenced
...
- `approval_policy` (Copilot SDK `AskForApproval` value)
  - Default: implementation-defined.
- `thread_sandbox` (Copilot SDK `SandboxMode` value)
  - Default: implementation-defined.
- `turn_sandbox_policy` (Copilot SDK `SandboxPolicy` value)
  - Default: implementation-defined.
...
- The software MUST attempt to adjust live behavior to the new config (for example polling
  cadence, concurrency limits, active/terminal states, copilot settings, workspace paths/hooks, and
...
- `copilot.command`: shell command string, default `copilot-sdk`
- `copilot.approval_policy`: Copilot SDK `AskForApproval` value, default implementation-defined
- `copilot.thread_sandbox`: Copilot SDK `SandboxMode` value, default implementation-defined
- `copilot.turn_sandbox_policy`: Copilot SDK `SandboxPolicy` value, default implementation-defined
- `copilot.turn_timeout_ms`: integer, default `3600000`
- `copilot.read_timeout_ms`: integer, default `5000`
...
- For each running issue, compute `elapsed_ms` since:
  - `last_copilot_timestamp` if any event has been seen, else
  - `started_at`
...
This section defines Symphony's language-neutral responsibilities when integrating the
Copilot SDK. The Copilot SDK protocol for the targeted Copilot SDK version is the source of truth for
protocol schemas, message payloads, transport framing, and method names.

Protocol source of truth:

- Implementations MUST send messages that are valid for the targeted Copilot SDK version.
- Implementations MUST consult the targeted Copilot SDK documentation or generated schema
  instead of treating this specification as a protocol schema.
- If this specification appears to conflict with the targeted Copilot SDK protocol, the Copilot SDK
...
- Invocation: `bash -lc <copilot.command>`
- Working directory: workspace path
- Transport/framing: the protocol transport required by the targeted Copilot SDK version

Notes:

- The default command is `copilot-sdk`.
- Approval policy, sandbox policy, cwd, prompt input, and OPTIONAL tool declarations are supplied
  using fields supported by the targeted Copilot SDK version.
...
Reference: Copilot SDK documentation.

Startup MUST follow the targeted Copilot SDK contract. Symphony additionally requires the
client to:

- Start the app-server subprocess in the per-issue workspace.
- Initialize the app-server session using the targeted Copilot SDK protocol.
...
- Extract `thread_id` from the thread identity returned by the targeted Copilot SDK protocol.
- Extract `turn_id` from each turn identity returned by the targeted Copilot SDK protocol.
- Emit `session_id = "<thread_id>-<turn_id>"`
- Reuse the same `thread_id` for all continuation turns inside one worker run

### 10.3 Streaming Turn Processing

The client processes app-server updates according to the targeted Copilot SDK protocol until
...
- Follow the transport and framing rules of the targeted Copilot SDK version.
...
- If implemented, supported tools SHOULD be advertised to the app-server session during startup
  using the protocol mechanism supported by the targeted Copilot SDK version.
...
- `copilot.read_timeout_ms`: request/response timeout during startup and sync requests
- `copilot.turn_timeout_ms`: total turn stream timeout
...
### 11.2 Query Semantics (GitHub Issues)

GitHub-specific requirements for `tracker.kind == "github"`:

- `tracker.kind == "github"`
- REST/GraphQL endpoint (default `https://api.github.com`)
- Auth token sent in `Authorization` header as `Bearer <token>`
- `tracker.repo` maps to a GitHub repository in `owner/repo` format
- Candidate issue query filters by repository and issue state (`open`)
...
- GitHub API schema details can drift. Keep query construction isolated and test the exact query
...
- `blocked_by` -> implementation-defined; GitHub Issues does not have native blocker relations
    ...
    ```json
    {
      ...
      "logs": {
        "copilot_session_logs": [
          {
            "label": "latest",
            "path": "/var/log/symphony/copilot/MT-649/latest.log",
            "url": null
          }
        ]
      },
...
Running Copilot SDK agents against repositories, issue trackers, and other inputs that can contain
...
- Tightening Copilot SDK approval and sandbox settings described elsewhere in this specification instead
  of running with a maximally permissive configuration.
- Adding external isolation layers such as OS/container/VM sandboxing, network restrictions, or
  separate credentials beyond the built-in Copilot SDK policy controls.
- Filtering which GitHub issues, repositories, labels, or other tracker sources are eligible for
...
function dispatch_issue(issue, state, attempt):
  ...
  state.running[issue.id] = {
    ...
    session_id: null,
    copilot_pid: null,
    last_copilot_message: null,
    last_copilot_event: null,
    last_copilot_timestamp: null,
...
function run_agent_attempt(issue, attempt, orchestrator_channel):
  ...
  while true:
    ...
    turn_result = app_server.run_turn(
      ...
      prompt=prompt,
      issue=issue,
      on_message=(msg) -> send(orchestrator_channel, {copilot_update, issue.id, msg})
    )
...
- GitHub Issues query uses the specified repository filter
- Empty `fetch_issues_by_states([])` returns empty without API call
- Pagination preserves order across multiple pages
- Blockers are handled per implementation-defined policy (GitHub Issues lacks native blockers)
- Labels are normalized to lowercase
- Issue state refresh by ID returns minimal normalized issues
- Issue state refresh query uses GitHub issue numbers as specified in Section 11.2
- Error mapping for request errors, non-200, API errors, malformed payloads
...
- Session startup follows the targeted Copilot SDK protocol.
- Client identity/capability payloads are valid when the targeted Copilot SDK protocol requires
  them.
...
- Copilot SDK launch command config (`copilot.command`, default `copilot-sdk`)
